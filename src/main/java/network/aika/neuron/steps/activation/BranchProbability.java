...
public class BranchProbability extends Step<BindingActivation> {
    ...
    public void process() { // TODO: Use branch binding signal
        ...
        double offset = conflictingActs
                .stream()
                .mapToDouble(cAct -> cAct.getNet().getOldValue())
                .min()
                .getAsDouble();

        double norm = Math.exp(act.getNet().getOldValue() - offset);
        norm += conflictingActs
                .stream()
                .mapToDouble(cAct -> Math.exp(cAct.getNet().getOldValue() - offset))
                .sum();

        double p = Math.exp(act.getNet().getOldValue() - offset) / norm;
