...
public class NodeActivation<T extends Node> implements Comparable<NodeActivation> {
    ...
    public static <T extends Node, A extends NodeActivation<T>> A get(Document doc, T n, Integer rid, Range r, Range.Relation rr, InterprNode o, InterprNode.Relation or) {
        Stream<A> s = select(doc, n, rid, r, rr, o, or);
        return s.findFirst()
                .orElse(null);
    }
    ...
    public static Stream<NodeActivation> select(Document doc, Integer rid, Range r, Range.Relation rr, InterprNode o, Relation or) {
    ...
    public static <T extends Node, A extends NodeActivation<T>> Stream<A> select(Document doc, T n, Integer rid, Range r, Range.Relation rr, InterprNode o, Relation or) {
    ...
    public static <T extends Node, A extends NodeActivation<T>> Stream<A> select(ThreadState<T, A> th, T n, Integer rid, Range r, Range.Relation rr, InterprNode o, Relation or) {
        Stream<A> results;
    ...
    public static <T extends Node, A extends NodeActivation<T>> Stream<A> getActivationsByRange(ThreadState<T, A> th, T n, Integer rid, Range r, Range.Relation rr, InterprNode o, InterprNode.Relation or) {
        Collection<A> s;
        if((rr.beginToBegin == GREATER_THAN_EQUAL || rr.beginToBegin == EQUALS) && r.begin != Integer.MIN_VALUE && r.begin <= r.end) {
            int er = (rr.endToEnd == Operator.LESS_THAN_EQUAL || rr.endToEnd == Operator.EQUALS) && r.end != Integer.MAX_VALUE ? r.end : Integer.MAX_VALUE;
            s = th.activations.subMap(
                    ...
                    .values();
        } else if((rr.beginToBegin == Operator.LESS_THAN_EQUAL || rr.beginToBegin == Operator.EQUALS) && r.begin != Integer.MIN_VALUE && r.begin <= r.end) {
        ...
        return s.stream().filter(act -> act.filter(n, rid, r, rr, o, or));
    }
