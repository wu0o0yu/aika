 ...
 */
public abstract class AbstractNeuron<T extends AbstractNeuron> extends AbstractNode<T> implements Comparable<AbstractNeuron> {
    ...
    public AbstractNeuron(String label) {
        this.label = label;
    }


    public AbstractNeuron(String label, boolean isBlocked, boolean noTraining) {
        this.label = label;
    ...
    public void remove(int threadId) {
        ...
        for(Synapse s: outputSynapses.values()) {
            AbstractNeuron out = s.output.get();
    ...
    public void train(Document doc, Activation iAct, Integer rid, double x, long v) {
        ...
        if(iiAct.key.n.neuron.get() != this && iiAct.finalState != null && iiAct.finalState.value > TOLERANCE) {
    ...
    public void count(Document doc) {
        ThreadState th = node.get().getThreadState(doc.threadId, false);
        if(th == null) return;
    ...
    private void linkNeuronActs(Document doc, Activation act, int v, int dir) {
        ...
        for (Synapse s : getActiveSynapses(doc, dir, syns)) {
            AbstractNeuron<?> an =  (dir == 0 ? s.input : s.output).get();
            Node n = an.node.get();
    ...
    private static void linkActSyn(Node n, Document doc, Activation act, int dir, ArrayList<Activation> recNegTmp, Synapse s) {
        Integer rid;
        if(dir == 0) {
    ...
    private static Collection<Synapse> getActiveSynapses(Document doc, int dir, TreeMap<Synapse, Synapse> syns) {
        ...
        for (AbstractNeuron n : doc.activatedNeurons) {
            if (dir == 0) {
                lk.input = n.provider;
                uk.input = n.provider;
            } else {
    ...
    public int compareTo(AbstractNeuron n) {
    ...
    public static AbstractNeuron read(DataInput in, Provider p) throws IOException {
        AbstractNeuron n = in.readBoolean() ? new InputNeuron() : new Neuron();
        n.provider = p;
        n.readFields(in, p.m);
        return n;
    }
    ...
    public Collection<Activation> getFinalActivations(Document doc) {
        return Activation.select(doc, node.get(), null, null, null, null, null, null)
    ...
    public static class NormWeight {
        ...
        public boolean equals(NormWeight nw) {
            return (Math.abs(w - nw.w) <= AbstractNeuron.WEIGHT_TOLERANCE && Math.abs(n - nw.n) <= AbstractNeuron.WEIGHT_TOLERANCE);
        }
