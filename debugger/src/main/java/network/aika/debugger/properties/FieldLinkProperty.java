...
public class FieldLinkProperty {
    ...
    public FieldLinkProperty(Container parent, AbstractFieldLink fl, Direction dir) {
        ...
        if(dir == INPUT) {
            fieldProperty = createFieldProperty(parent, fl.getInput(), true, fl.isConnected());
        } else {
            if(fl instanceof FieldLink) {
                fieldProperty = createFieldProperty(parent, (FieldOutput) fl.getOutput(), true, fl.isConnected());
